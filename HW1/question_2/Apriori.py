...
updating get_triple(). see in Apriori0.py
'''
...
class Apriori():
    ...
    def get_triple(self,file):
        '''first generate triples from doubles, then scan through input to count triples\
        finally prune the unfrequent triples     '''
        #generate triples
        s = set()
        ...
        triple = itertools.combinations(s,3)
        list_triple = [sorted(elem) for elem in triple]

        #print(list_triple)
        #input()

        #then scan through input to count triples
        for line in self.loadfile(file):
            s = set()
            for item in line:
                s.add(item)
            ...
            if len(s) > 2:
                ...
                for elem in triple:
                    ...
                    #print(elem)
                    #input()
                    if elem in list_triple:
                        self.items_triple.setdefault(elem[0],{})
                        self.items_triple[elem[0]].setdefault(elem[1],{})
                        self.items_triple[elem[0]][elem[1]].setdefault(elem[2],0)
                        self.items_triple[elem[0]][elem[1]][elem[2]] += 1
